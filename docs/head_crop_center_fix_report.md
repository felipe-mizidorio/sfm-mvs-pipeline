...
## Sentinel finding (expected on this phantom, out of scope here)

The clamp sentinel **fired** on this run: median corner distance from the centroid is 185 mm, +100 mm margin = 285 mm > 250 mm ceiling. This is the expected outcome for this capture target: the test object is an **adult-size ~250 mm mannequin phantom, not a neonatal head**, so the marker cloud genuinely spans ~2× the neonatal envelope. On a neonatal-sized head (corners ≲ 80 mm from the centroid) the auto radius lands around 180 mm and the clamp stays silent — which is what T2's "clean run" acceptance refers to. The recovered scale itself has been cross-checked before (marker sides measure 44.6–52.9 mm at ~146 mm/unit on this phantom), but **independent** metric validation still requires a measured rigid cap layout (`known_distances_mm` is empty, so the A3 layout check remains inert). Until then, do not tune Taubin/density thresholds against this run's mm values, and expect the sentinel to keep firing on this oversized phantom.
